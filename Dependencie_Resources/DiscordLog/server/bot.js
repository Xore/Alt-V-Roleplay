...
/*Client.on("ready", async() => {
    ...
    try {
        ...
        const activity = server.data.info.players;
        Client.user.setPresence({ activity: { type: "PLAYING", name: `${activity} | Online🟢` }, status: 'online' })
    } catch (e) {
        Client.user.setPresence({ activity: { type: "WATCHING", name: `Offline🔴` }, status: 'online' })
...
Client.on('message', async message => {
    ...
    if (talkedRecently.has(message.author.id)) {} else {

        if (message.content.toLowerCase() === `${prefix}status`) {
            bannerUrl = "https://abload.de/img/logo_no-bgynjfy.png";
            ...
            let serverstatsoffline = new Discord.MessageEmbed()
                ...
                .addFields({ name: 'Description', value: `Offline 🔴` }, { name: 'Tags', value: `Offline 🔴\n` }, { name: 'Version', value: `Offline 🔴`, inline: true }, { name: 'Language', value: `Offline 🔴`, inline: true }, { name: 'Game Mode', value: `Offline 🔴`, inline: true }, )
                .setImage(`${bannerUrl}`)
            ...
            try {
                ...
                let serverstatsonline = new Discord.MessageEmbed()
                    ...
                    .setThumbnail(`${bannerUrl}`)
                    .addFields({ name: 'Description', value: `${server.data.info.description}` }, { name: 'Tags', value: `${server.data.info.tags.length == 0 ? "Not set!" : server.data.info.tags }\n` }, { name: 'Version', value: `${server.data.info.version.length == 0 ? "Not set!" : server.data.info.version}`, inline: true }, { name: 'Language', value: `${server.data.info.language.length == 0 ? "Not set!" : server.data.info.language}`, inline: true }, { name: 'Game Mode', value: `${server.data.info.gameMode.length == 0 ? "Not set!" : server.data.info.gameMode}`, inline: true }, )
                    .setImage(`${bannerUrl}`)
