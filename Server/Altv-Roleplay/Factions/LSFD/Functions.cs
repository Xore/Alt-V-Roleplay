    ...
    public class Functions : IScript
    {
        ...
        public async void RevivePlayer(IPlayer player, IPlayer targetPlayer)
        {
            try
            {
                ...
                if(!CharactersInventory.ExistCharacterItem(charId, "Defibrilator", "inventory") && !CharactersInventory.ExistCharacterItem(charId, "Defibrilator", "backpack")) { HUDHandler.SendNotification(player, 4, 5000, "Du hast keinen Defibrilator dabei."); return; }
                ...
                else if(rnd >= 51 && rnd <= 100)
                {
                    ...
                    if (!player.IsInVehicle) InventoryHandler.StopAnimation(player, "missheistfbi3b_ig8_2", "cpr_loop_paramedic");
                    HUDHandler.SendNotification(player, 4, 5000, "Die Reanimation war nicht erfolgreich, versuch es weiter!");
                }
            }
            catch (Exception e)
            {
        ...
        public async void HealPlayer(IPlayer player, IPlayer targetPlayer)
        {
            try
            {
                ...
                if(!CharactersInventory.ExistCharacterItem(charId, "Verbandskasten", "inventory") && !CharactersInventory.ExistCharacterItem(charId, "Verbandskasten", "backpack")) { HUDHandler.SendNotification(player, 4, 5000, "Du hast keinen Verbandskasten dabei."); return; }
                ...
                HUDHandler.SendNotification(player, 2, 5000, "Spieler erfolgreich behandelt.");
                HUDHandler.SendNotification(targetPlayer, 1, 5000, "Ein Arzt hat dich behandelt.");
