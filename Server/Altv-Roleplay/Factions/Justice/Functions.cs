    ...
    class Functions : IScript
    {
        ...
        public void GiveWeaponLicense(IPlayer player, string targetCharName)
        {
            try
            {
                ...
                if (ServerFactions.GetCharacterFactionId(charId) != 1) { HUDHandler.SendNotification(player, 3, 2500, "Du bist kein Angehöriger der Justiz."); return; }
                if (!Characters.ExistCharacterName(targetCharName)) { HUDHandler.SendNotification(player, 3, 2500, $"Der angegebene Name wurde nicht gefunden ({targetCharName})."); return; }
                ...
                if (!player.Position.IsInRange(targetPlayer.Position, 5f)) { HUDHandler.SendNotification(player, 3, 2500, "Der Spieler ist nicht in Ihrer Nähe."); return; }
                if (CharactersLicenses.HasCharacterLicense(targetCharId, "weaponlicense")) { HUDHandler.SendNotification(player, 3, 2500, "Der Spieler hat bereits einen Waffenschein."); return; }
                CharactersLicenses.SetCharacterLicense(targetCharId, "weaponlicense", true);
                HUDHandler.SendNotification(player, 1, 2500, $"Sie haben dem Spieler {targetCharName} den Waffenschein erfolgreich ausgestellt.");
                HUDHandler.SendNotification(targetPlayer, 2, 2500, $"Ihnen wurde der Waffenschein erfolgreich ausgestellt.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void SearchBankAccounts(IPlayer player, string targetCharName)
        {
            try
            {
                ...
                if (targetBankAccounts == "[]" || targetBankAccounts == "") { HUDHandler.SendNotification(player, 3, 2500, "Der Spieler hat keine Bankkonten."); return; }
        ...
        public void ViewBankTransactions(IPlayer player, int accNumber)
        {
            try
            {
                ...
                if (!ServerFactions.IsCharacterInFactionDuty(charId)) { HUDHandler.SendNotification(player, 3, 2500, "Du bist nicht im Dienst."); return; }
                if (ServerFactions.GetCharacterFactionId(charId) != 1) { HUDHandler.SendNotification(player, 3, 2500, "Du bist kein Angehöriger der Justiz."); return; }
                if (!CharactersBank.ExistBankAccountNumber(accNumber)) { HUDHandler.SendNotification(player, 3, 2500, $"Die ausgewählte Kontonummer existiert nicht ({accNumber})."); return; }
                var bankPapers = ServerBankPapers.GetTabletBankAccountBankPaper(accNumber);
                if (bankPapers == "[]" || bankPapers == "") { HUDHandler.SendNotification(player, 3, 2500, "Dieses Konto besitzt keine Transaktionen."); return; }
