    ...
    class Functions : IScript
    {
        ...
        public void RepairVehicle(IPlayer player, IVehicle vehicle)
        {
            try
            {
                ...
                if(!CharactersInventory.ExistCharacterItem(charId, "Reparaturkit", "inventory") && !CharactersInventory.ExistCharacterItem(charId, "Reparaturkit", "backpack")) { HUDHandler.SendNotification(player, 3, 2500, "Du besitzt kein Reparaturkit."); return; }
        ...
        [AsyncClientEvent("Server:Raycast:WashVehicle")]
        public void WashVehicle(IPlayer player, IVehicle vehicle)
        {
            try
            {
                if (player == null || !player.Exists || vehicle == null || !vehicle.Exists) return;
                int charId = (int)player.GetCharacterMetaId();
                if (charId <= 0 || player.HasPlayerRopeCuffs() || player.HasPlayerHandcuffs() || player.IsPlayerUnconscious()) return;
                if (!CharactersInventory.ExistCharacterItem(charId, "Lappen", "inventory") && !CharactersInventory.ExistCharacterItem(charId, "Lappen", "backpack")) { HUDHandler.SendNotification(player, 3, 2500, "Du besitzt kein Lappen."); return; }
                if (CharactersInventory.ExistCharacterItem(charId, "Lappen", "inventory")) CharactersInventory.RemoveCharacterItemAmount(charId, "Lappen", 1, "inventory");
                else if (CharactersInventory.ExistCharacterItem(charId, "Lappen", "backpack")) CharactersInventory.RemoveCharacterItemAmount(charId, "Lappen", 1, "backpack");
                //ToDo: Reparatur-Animation abspielen < Anim call in HUD.js
                ServerVehicles.SetVehicleEngineHealthy(vehicle, true);
                Alt.EmitAllClients("Client:Utilities:washVehicle", vehicle);
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void TowVehicle(IPlayer player, IVehicle vehicle)
        {
            try
            {
                ...
                if (ServerFactions.GetCharacterFactionId(charId) == 4 && ServerFactions.GetCharacterFactionId(charId) == 2 && ServerFactions.GetCharacterFactionId(charId) == 12) return;
                ...
                switch(vehClass)
                {
                    case 0: //Fahrzeuge
                        ...
                        break;
                    case 1: //Boote
                        ServerVehicles.SetVehicleInGarage(vehicle, true, 16);
                        break;
                    case 2: //Flugzeuge
                        ServerVehicles.SetVehicleInGarage(vehicle, true, 14);
                        break;
                    case 3: //Helikopter
                        ServerVehicles.SetVehicleInGarage(vehicle, true, 15);
                        break;
                }              
                ServerFactions.SetFactionBankMoney(4, ServerFactions.GetFactionBankMoney(4) + 1500); //ToDo: Anpassen
                HUDHandler.SendNotification(player, 2, 2500, "Fahrzeug erfolgreich verwahrt.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void buyTuningPart(IPlayer player, IVehicle vehicle, int type, int index)
        {
            try
            {
                ...
                //if (!CharactersInventory.ExistCharacterItem(charId, "Bargeld", "brieftasche") || CharactersInventory.GetCharacterItemAmount(charId, "Bargeld", "brieftasche") < price) HUDHandler.SendNotification(player, $"Du hast nicht genug Bargeld dabei (${price}).", "alert", 5000);
