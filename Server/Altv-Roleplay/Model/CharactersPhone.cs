...
namespace Altv_Roleplay.Model
{
    class CharactersPhone : IScript
    {
        ...
        public static void CreatePhoneVerlauf(ClassicPlayer player, int phoneNumber, int targetPhoneNumber)
        {
            try
            {
                foreach (var ev in CharactersPhone.CharactersPhoneVerlauf_.Where(x => DateTime.Now.Subtract(Convert.ToDateTime(x.date)).TotalHours >= 96).ToList())
                {
                    using (gtaContext db = new gtaContext())
                    {
                        var dbEntry = db.CharactersPhoneVerlauf.Find(ev.id);
                        if (dbEntry == null) return;
                        db.CharactersPhoneVerlauf.Remove(dbEntry);
                        CharactersPhone.CharactersPhoneVerlauf_.Remove(ev);
                        db.SaveChanges();
                    }
                }
        ...
        public static void GetCallHistory(int charId)
        {
            try
            {
                using (var db = new gtaContext())
                {
                    CharactersPhone.CharactersPhoneVerlauf_ = new List<CharactersPhoneVerlauf>(db.CharactersPhoneVerlauf.OrderByDescending(x => x.date));
                }
            }
            catch (Exception e)
            {
