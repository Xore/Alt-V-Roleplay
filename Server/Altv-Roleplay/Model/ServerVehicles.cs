...
namespace Altv_Roleplay.Model
{
    class ServerVehicles
    {
        ...
        public static int GetVehiclePriceOnHash(long hash)
        {
            return ServerAllVehicles.ServerAllVehicles_.FirstOrDefault(x => x.hash == hash)?.price ?? 0;
        }
        ...
        public static void SetServerVehicleOwner(int vehID, int charid)
        {
            try
            {
        ...
        public static void RemoveVehiclePermanently(IVehicle veh)
        {
            try
            {
                ...
                using (gtaContext db = new gtaContext())
                {
                    if (mod == null || vehDb == null) return;
                    var dbMod = db.Server_Vehicles_Mods.Find(mod.id);
                    var dbvehDb = db.Server_Vehicles.Find(vehDb.id);
                    if (dbMod == null || dbvehDb == null) return;
                    ...
                    foreach (var item in ServerVehicleTrunkItems_.Where(x => x.vehId == (int)vehID).ToList())
                    {
                        if (item == null) return;
                        var dbitem = db.Server_Vehicle_Items.Find(item.id);
