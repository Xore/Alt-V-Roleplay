    ...
    class ServerHouses
    {
        ...
        internal static void CreateHouse(int id, int interiorId, int ownerId, string street, int price, int maxRenters, int rentPrice, bool isRentable, bool hasStorage, bool hasAlarm, bool hasBank, Position entrancePos, int money)
        {
            try
            {
                ...
                int blipColor = 0;
                string blipName = "Haus [Frei]";
                if (ownerId != 0) { blipColor = 3; } { blipName = "Haus [Im Besitz/Mietbar]"; }
        ...
        public static int GetHousesOwned(int charId)
        {
            try
            {
                if (charId <= 0) return 0;
                var house = ServerHouses_.Where(x => x.ownerId == charId);
                if (house != null) return house.Count();
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
