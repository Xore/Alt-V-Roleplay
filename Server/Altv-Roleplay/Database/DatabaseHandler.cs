    ...
    internal class DatabaseHandler
    {
        ...
        internal static void LoadAllPlayers()
        {
            try
            {
                ...
                {
                    foreach (var player in db.Accounts)
                    {
                        if (player.whitelistTime != 0)
                        {
                            player.whitelistTime = 0;
                            db.Accounts.Update(player);
                            Alt.Log("Whitelist Time set to 0");
                        }
                    }
                    db.SaveChanges();
                }
            ...
            try
            {
                using (var db = new gtaContext())
                {
                    ...
                    foreach (var shop in ServerShops.ServerShops_)
                    {
                        if (shop.id == 31)
                        {
                            shop.pedPos = Constants.DealerPositions.Dealer_1;
                            shop.pedRot = Constants.DealerPositions.Dealer_1_Rot;
                            shop.shopPos = Constants.DealerPositions.Dealer_1;
        ...
        /*internal static void ResetWhitelistTime() //On Server Start whitelistTime is set to "0", after the Client Connects it hops back to not "0" xx
        {
            try
            {
                using (var db = new gtaContext())
