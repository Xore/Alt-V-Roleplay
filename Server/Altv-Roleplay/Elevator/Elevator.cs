    ...
    class KeyHandler : IScript
    {
        ...
        public void PressE(IPlayer player)
        {
            lock (player)
            {
                ...
                if (charId <= 0) return;
                if (player.HasPlayerHandcuffs() || player.HasPlayerRopeCuffs()) { HUDHandler.SendBetterNotif(player, 3, 10, "Fahrstuhl", $"Wie willst du das mit Handschellen/Fesseln machen?"); return; }
                {
