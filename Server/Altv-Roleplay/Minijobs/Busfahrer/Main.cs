    ...
    public class Main : IScript
    {
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) { HUDHandler.SendNotification(player, 3, 2500, $"Dein Gehalt i.H.v. {givenMoney}$ konnte nicht überwiesen werden da du kein Hauptkonto hast. Du hast aber {givenEXP}EXP erhalten (du hast nun: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP)."); return; }
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {givenMoney}$ wurde dir auf dein Hauptkonto überwiesen. Du hast {givenEXP} erhalten (deine EXP: {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")})");
                return;
            }
            catch (Exception e)
            {
        ...
        private static void PlayerEnterVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                if (player.GetPlayerCurrentMinijobStep() == "FirstStepInVehicle")
                {
                    ...
                    if (spot == null) return;
                    HUDHandler.SendNotification(player, 1, 2500, "Fahre zur ersten Haltestelle und warte dort 10 Sekunden.");
        ...
        private static void ColshapeEnterExitHandler(IColShape colShape, IEntity targetEntity, bool state)
        {
            try
            {
                ...
                if (colShape == startJobShape && state)
                {
                    ...
                    else if (client.GetPlayerCurrentMinijob() != "None") { HUDHandler.SendNotification(client, 3, 2500, "Du bist bereits in einem Minijob."); }
                    return;
                }
                ...
                if (client.GetPlayerCurrentMinijobStep() == "DRIVE_TO_NEXT_STATION" && state && client.IsInVehicle)
                {
                    ...
                    if ((int)client.GetPlayerCurrentMinijobActionCount() < maxSpots)
                    {
                        ...
                        HUDHandler.SendNotification(client, 1, 2500, "An Haltestelle angekommen, warte 10 Sekunden und fahre anschließend zur nächsten Haltestelle.");
                    ...
                    {
                        //zurueck zum Depot
                        HUDHandler.SendNotification(client, 1, 2500, "An Haltestelle angekommen, warte 10 Sekunden und fahre den Bus anschließend zurück zum Depot und stelle ihn dort ab, wo du ihn bekommen hast.");
        ...
        internal static void TryStartMinijob(IPlayer player)
        {
            try
            {
                ...
                if (player.GetPlayerCurrentMinijob() == "Busfahrer")
                {
                    ...
                    HUDHandler.SendNotification(player, 1, 2500, "Du hast den Minijob: Busfahrer beendet.");
                    player.SetPlayerCurrentMinijob("None");
                    player.SetPlayerCurrentMinijobRouteId(0);
                    player.SetPlayerCurrentMinijobStep("None");
                    player.SetPlayerCurrentMinijobActionCount(0);
        ...
        public void StartMiniJob(IPlayer player, int routeId)
        {
            try
            {
                ...
                if (CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer") < Model.GetRouteNeededEXP(routeId)) { HUDHandler.SendNotification(player, 3, 2500, $"Du hast nicht die benötigen EXP für diese Linie ({Model.GetRouteNeededEXP(routeId)}EXP - du hast {CharactersMinijobs.GetCharacterMinijobEXP(charId, "Busfahrer")}EXP)."); return; }
