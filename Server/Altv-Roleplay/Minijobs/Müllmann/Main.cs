    ...
    public class Main
    {
        ...
        private static async void PlayerExitVehicle(IVehicle vehicle, IPlayer player, byte seat)
        {
            try
            {
                ...
                if (!CharactersBank.HasCharacterBankMainKonto(charId)) { HUDHandler.SendNotification(player, 3, 2500, $"Dein Gehalt i.H.v. {rnd}$ konnte nicht überwiesen werden da du kein Hauptkonto hast."); return; }
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Du hast den Minijob erfolgreich abgeschlossen. Dein Gehalt i.H.v. {rnd}$ wurde dir auf dein Hauptkonto überwiesen.");
                return;
            }
            catch (Exception e)
            {
        ...
        private static void ColshapeEnterExitHandler(IColShape colShape, IEntity targetEntity, bool state)
        {
            try
            {
                ...
                if (client.GetPlayerCurrentMinijobStep() == "PICKUP_TRASH" && state && !client.IsInVehicle)
                {
                    ...
                    //InventoryHandler.InventoryAnimation(client, "Trash", -1);
                    client.EmitLocked("Client:Inventory:AttachObject", "trash");
                    var veh = Alt.GetAllVehicles().ToList().FirstOrDefault(x => x.Exists && x.NumberplateText == $"MM-{charId}");
                    if (veh == null || !veh.Exists) return;
                    HUDHandler.SendNotification(client, 1, 2500, "Mülltonne geleert, werfe den Müll in den Wagen.");
                ...
                else if (client.GetPlayerCurrentMinijobStep() == "THROW_TRASH_IN_VEHICLE" && state && !client.IsInVehicle)
                {
                    ...
                    client.EmitLocked("Client:Minijob:RemoveJobMarker");
                    InventoryHandler.InventoryAnimation(client, "Trash2", 0);
                    InventoryHandler.DetachObject(client);
                    ...
                    else if ((int)client.GetPlayerCurrentMinijobActionCount() >= maxSpots)
                    {
                        //zurueck zum Depot
                        if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                        HUDHandler.SendNotification(client, 1, 2500, "Alles aufgesammelt mein Jung. Zurück zur Mülldeponie - das Zeug abgeben, stell das Fahrzeug einfach dort ab wo du es bekommen hast.");
        ...
        internal static void StartMinijob(IPlayer player)
        {
            try
            {
                ...
                if (player.GetPlayerCurrentMinijob() == "Müllmann")
                {
                    ...
                    if (personalThrowCol != null && personalThrowCol.Exists) personalThrowCol.Remove();
                    HUDHandler.SendNotification(player, 1, 2500, "Du hast den Minijob: Müllmann beendet.");
                    player.SetPlayerCurrentMinijob("None");
                    player.SetPlayerCurrentMinijobStep("None");
                    player.SetPlayerCurrentMinijobRouteId(0);
                    player.SetPlayerCurrentMinijobActionCount(0);
