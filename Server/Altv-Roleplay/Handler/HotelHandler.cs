    ...
    class HotelHandler : IScript
    {
        ...
        public void RentHotel(IPlayer player, int hotelId, int apartmentId)
        {
            try
            {
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Sie haben sich das Zimmer mit der Zimmernummer '{apartmentId}' erfolgreich gemietet (Kosten: {ServerHotels.GetApartmentPrice(hotelId, apartmentId)}$). Dieses Zimmer läuft automatisch nach {ServerHotels.GetApartmentRentHours(hotelId, apartmentId)} Stunden ab.");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void StorageHotelItem(IPlayer player, int apartmentId, string itemName, int itemAmount, string fromContainer)
        {
            try
            {
                ...
                if (!CharactersInventory.ExistCharacterItem(cCharId, itemName, fromContainer)) { HUDHandler.SendNotification(player, 3, 2500, "Fehler: Diesen Gegenstand besitzt du nicht."); return; }
                ...
                HUDHandler.SendNotification(player, 1, 2500, $"Der Gegenstand wurde erfolgreich eingelagert ({itemName} - {itemAmount}x).");
