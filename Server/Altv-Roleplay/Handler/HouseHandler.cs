    ...
    class HouseHandler : IScript
    {
        ...
        public void BuyHouse(IPlayer player, int houseId)
        {
            try
            {
                ...
                if (ServerHouses.GetHousesOwned(charId) > 10) { HUDHandler.SendNotification(player, 3, 2500, "Dir gehören bereits 10 Häuser!"); return; }
                if (ServerHouses.GetHouseOwner(houseId) > 0) { HUDHandler.SendNotification(player, 3, 2500, "Dieses Haus gehört bereits jemanden."); return; }
                ...
                if (CharactersBank.GetBankAccountMoney(accNumber) < ServerHouses.GetHousePrice(houseId)) { HUDHandler.SendNotification(player, 3, 2500, $"Soviel Geld hast du auf deinem Konto nicht ({ServerHouses.GetHousePrice(houseId)}$) - du hast {CharactersBank.GetBankAccountMoney(accNumber)}$"); return; }
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Sie haben sich das Haus mit der Adresse '{ServerHouses.GetHouseStreet(houseId)}' erfolgreich gekauft (Kosten: {ServerHouses.GetHousePrice(houseId)}$).");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void StorageItem(IPlayer player, int houseId, string itemName, int itemAmount, string fromContainer)
        {
            try
            {
                ...
                if (!CharactersInventory.ExistCharacterItem(charId, itemName, fromContainer)) { HUDHandler.SendNotification(player, 4, 2500, "Diesen Gegenstand besitzt du nicht."); return; }
                if (CharactersInventory.GetCharacterItemAmount(charId, itemName, fromContainer) < itemAmount) { HUDHandler.SendNotification(player, 4, 2500, "Du hast nicht genügend Gegenstände davon dabei."); return; }
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Der Gegenstand wurde erfolgreich eingelagert ({itemName} - {itemAmount}x).");
                //ToDo: Log Eintrag
            }
            catch (Exception e)
            {
        ...
        public void TakeItem(IPlayer player, int houseId, string itemName, int itemAmount)
        {
            try
            {
                ...
                if (charId <= 0) return;
                if (!ServerHouses.ExistHouse(houseId)) return;
                if (!ServerHouses.HasHouseStorageUpgrade(houseId)) { HUDHandler.SendNotification(player, 3, 2500, "Dieses Haus besitzt noch keinen ausgebauten Lagerplatz."); return; }
                if (player.Dimension - 10000 <= 0 || player.Dimension - 10000 != houseId) return;
                if (player.HasPlayerHandcuffs() || player.HasPlayerRopeCuffs()) { HUDHandler.SendNotification(player, 3, 2500, "Wie willst du das mit Handschellen/Fesseln machen?"); return; }
                if (!ServerHouses.ExistServerHouseStorageItem(houseId, itemName)) { HUDHandler.SendNotification(player, 3, 2500, "Der Gegenstand existiert im Hauslager nicht."); return; }
                if (ServerHouses.GetServerHouseStorageItemAmount(houseId, itemName) < itemAmount) { HUDHandler.SendNotification(player, 4, 2500, "Soviele Gegenstände sind nicht im Hauslager."); return; }
                float itemWeight = ServerItems.GetItemWeight(itemName) * itemAmount;
        ...
        internal static async void BreakIntoHouse(IPlayer player, int houseId)
        {
            ...
            try
            {
                ...
                if (!player.IsPlayerUsingCrowbar())
                {
                    ...
                    //Aufbrechen Starten
                    if (DateTime.Now.Subtract(Convert.ToDateTime(Characters.GetCharacterLastLogin(houseOwner))).TotalHours >= 48) { HUDHandler.SendNotification(player, 3, 2500, "Der Hausbesitzer war in den letzten 48 Stunden nicht da."); return; }
                    ...
                    if (ServerHouses.HasHouseAlarmUpgrade(houseId))
                    {
                        ...
                        {
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 2500, $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 2500, $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                            HUDHandler.SendNotification(houseOwnerPlayer, 3, 2500, $"Jemand bricht in dein Haus ein: {ServerHouses.GetHouseStreet(houseId)}");
                        }
                    }
                ...
                else
                {
                    ...
                    HUDHandler.SendNotification(player, 3, 2500, "Du hast den Einbruch abgebrochen.");
                    player.SetPlayerUsingCrowbar(false);
                    return;
                }
            }
            ...
            try
            {
                ...
                switch (upgrade)
                {
                    case "alarm":
                        ...
                        return;
                    case "storage":
                        if (ServerHouses.HasHouseStorageUpgrade(houseId)) { HUDHandler.SendNotification(player, 1, 2500, "Dein Haus besitzt bereits eine Lagermöglichkeit."); return; }
                        if (!ServerHouses.HasHouseBankUpgrade(houseId)) { HUDHandler.SendNotification(player, 3, 2500, "Du hast noch keinen Tresor ausgebaut in dem genügend Geld ist (1500$)."); return; }
                        if (ServerHouses.GetHouseBankMoney(houseId) < 1500) { HUDHandler.SendNotification(player, 3, 2500, "Dein Haustresor verfügt nicht über die Kosten (1500$)."); return; }
        ...
        public void RentHouse(IPlayer player, int houseId)
        {
            try
            {
                ...
                HUDHandler.SendNotification(player, 2, 2500, $"Du hast dich erfolgreich eingemietet, die Miete beträgt {rentPrice}$");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
        ...
        public void UnrentHouse(IPlayer player, int houseId)
        {
            try
            {
                ...
                HUDHandler.SendNotification(player, 2, 2500, "Du hast den Mietvertrag gekündigt");
            }
            catch (Exception e)
            {
                Alt.Log($"{e}");
