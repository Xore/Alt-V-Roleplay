                            ...
                            if (charId > 0)
                            {
                                ...
                                if (Characters.IsCharacterUnconscious(charId))
                                {
                                    ...
                                    else if (unconsciousTime <= 0)
                                    {
                                        ...
                                        {
                                            //MALE
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 3, 0, 0);//Torso
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 4, 61, 0);//Legs
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 6, 6, 0);//Shoes
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 8, 15, 0);//Undershirt
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 11, 80, 0);//Tops
                                        }
                                        else if (Characters.GetCharacterGender(charId) == true)
                                        {
                                            //FEMALE
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 3, 2, 0);//Torso
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 4, 2, 0);//Legs
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 6, 13, 0);//Shoes
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 8, 15, 0);//Undershirt
                                            player.EmitLocked("Client:SpawnArea:setCharClothes", 11, 2, 0);//Tops
                                        }
                                    }
                                }
                                    ...
                                    else if (jailTime <= 0)
                                    {
                                        if (CharactersWanteds.HasCharacterWanteds(charId))
                                        {
                                            ...
                                            if (CharactersBank.HasCharacterBankMainKonto(charId))
                                            {
                                                ...
                                                HUDHandler.SendNotification(player, 3, 2500, $"Durch deine Inhaftierung wurden dir {jailPrice}$ vom Konto abgezogen.");
                                            }
                                            HUDHandler.SendNotification(player, 3, 2500, $"Du sitzt nun für {jailTimes} Minuten im Gefängnis.");
                                            HUDHandler.SendNotification(player, 3, jailTimes, $"Du bist noch in Haft!");
                                            ...
                                            {
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 3, 0, 0);//Torso
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 4, 3, 7);//Legs
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 6, 1, 1);//Shoes
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 8, 15, 0);//Undershirt
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 11, 22, 0);//Tops
                                            }
                                            else
                                            {
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 3, 4, 0);//Torso
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 4, 3, 15);//Legs
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 6, 1, 6);//Shoes
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 8, 15, 0);//Undershirt
                                                player.EmitLocked("Client:SpawnArea:setCharClothes", 11, 118, 0);//Tops
                                            }
                                        }
                                        else
                                        {
                                    ...
                                    if (CharactersBank.HasCharacterBankMainKonto(charId))
                                    {
                                        ...
                                        if (!Characters.IsCharacterCrimeFlagged(charId) && Characters.GetCharacterJob(charId) != "None" && DateTime.Now.Subtract(Convert.ToDateTime(Characters.GetCharacterLastJobPaycheck(charId))).TotalHours >= 12 && !ServerFactions.IsCharacterInAnyFaction(charId))
                                        {
                                            if (Characters.GetCharacterJobHourCounter(charId) >= ServerJobs.GetJobNeededHours(Characters.GetCharacterJob(charId)) - 1)
                                            {
                                                ...
                                                HUDHandler.SendNotification(player, 1, 2500, $"Gehalt erhalten (Beruf: {Characters.GetCharacterJob(charId)} | Gehalt: {jobCheck}$)");
                                            }
                                        ...
                                        if (ServerFactions.IsCharacterInAnyFaction(charId) && ServerFactions.IsCharacterInFactionDuty(charId))
                                        {
                                            ...
                                            if (ServerFactions.GetFactionBankMoney(factionid) >= factionPayCheck)
                                            {
                                                ...
                                                HUDHandler.SendNotification(player, 1, 2500, $"Du hast deinen Lohn i.H.v. {factionPayCheck}$ erhalten ({ServerFactions.GetFactionRankName(factionid, ServerFactions.GetCharacterFactionRank(charId))})");
                                            ...
                                            {
                                                HUDHandler.SendNotification(player, 3, 2500, $"Deine Fraktion hat nicht genügend Geld um dich zu bezahlen ({factionPayCheck}$).");
                                            }
                                        }

                                        var playerVehicles = ServerVehicles.ServerVehicles_.Where(x => x.id > 0 && x.charid == charId && x.plate.Contains("LS"));
                                        int taxMoney = 0;
                                        foreach (var i in playerVehicles)
                                        {
                                            if (!i.plate.Contains("LS")) continue;
                                            taxMoney += ServerAllVehicles.GetVehicleTaxes(i.hash);
                                        }
                                    }
                                    else { HUDHandler.SendNotification(player, 3, 2500, $"Dein Einkommen konnte nicht überwiesen werden da du kein Hauptkonto hast."); }
                                }
                            }
                        }
                    }
                    ...
                    lock (player)
                    {
                        if (player.Exists && User.GetPlayerOnline(player) != 0)
                        {
                            ...
                            else
                            {
                                ...
                                Characters.SetCharacterHealth(charId, player.Health);
                                //HUDHandler.SendNotification(player, $"Du hast Hunger.", "info", 2500);
                            }
                            ...
                            else
                            {
                                ...
                                Characters.SetCharacterHealth(charId, player.Health);
                                //HUDHandler.SendNotification(player, $"Du hast Durst.", "info", 2500);
                            }
