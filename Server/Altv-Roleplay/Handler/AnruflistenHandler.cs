    ...
    public class AnruflistenHandler : IScript
    {
        ...
        public static void Load()
        {
            using (var db = new gtaContext())
            {
                ...
                Console.WriteLine($"{ServerCalls_.Count} Anruflisten-Verläufe geladen");
            }
        }

        public static void AddCallHistory(int charId, int number, string result, DateTime timestamp)
        {
            try
            {
                if (charId <= 0 || number <= 0) return;
        ...
        public static void RefreshList(ClassicPlayer player)
        {
            ...
            var items = ServerCalls_.ToList().Where(x => x.charId == player.CharacterId).Select(x => new
            {
                ...
                x.result,
                x.timestamp,
            }).OrderBy(x => x.id).TakeLast(20).ToList();
