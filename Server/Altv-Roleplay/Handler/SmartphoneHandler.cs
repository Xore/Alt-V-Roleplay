    ...
    class SmartphoneHandler : IScript
    {
            ...
            try
            {
                ...
                if (Characters.GetCharacterPhoneTargetNumber(player.CharacterId) != 0)
                {
                    ...
                    if (targetPlayer != null && targetPlayer.Exists && targetPlayer.CharacterId > 0)
                    {
                        ...
                        Characters.SetCharacterCurrentlyRecieveCallState(targetPlayer.CharacterId, 0);
                        targetPlayer.EmitLocked("Client:Smartphone:ShowPhoneCallError", 4);
                        HUDHandler.SendNotification(targetPlayer, 1, 2500, "Anruf beendet");
                    }
                    ...
                    player.EmitLocked("Client:Smartphone:ShowPhoneCallError", 4);
                    HUDHandler.SendNotification(player, 1, 2500, "Anruf beendet");
                    return;
                }
            }
            catch (Exception e)
        ...
        public async Task requestPhonebusiness(IPlayer player)
        {
            try
            {
                ...
                int gangID = ServerGangs.GetCharacterGangId((int)player.GetCharacterMetaId());
                var business = ServerGangs.ServerGangMembers_.ToList().Where(x => x.gangId == gangID).Select(x => new
                {
                    x.charId,
                    x.rank
        ...
        public static void requestPhoneVerlauf(ClassicPlayer player)
        {
            try
            {
                ...
                if (charId <= 0) return;
                CharactersPhone.GetCallHistory(charId);
                var verlaufsListe = CharactersPhone.CharactersPhoneVerlauf_.ToList();
                var itemCount = (int)verlaufsListe.Count;
                var iterations = Math.Floor((decimal)(itemCount / 10));
                var rest = itemCount % 10;
                for (var i = 0; i < iterations; i++)
                {
                    var skip = i * 10;
                    player.EmitLocked("Client:Smartphone:addVerlaufJSON", JsonConvert.SerializeObject(verlaufsListe.Skip(skip).Take(10).ToList()));
                }

                if (rest != 0) player.EmitLocked("Client:Smartphone:addVerlaufJSON", JsonConvert.SerializeObject(verlaufsListe.Skip((int)iterations * 10).ToList()));
        ...
        {
            //ToDo: Change to input value e.g: ticketfine = 1337$ + jailtime = 123(minutes)
            try
            {
                if (player == null || !player.Exists || player.CharacterId <= 0) return;
        ...
        public void GiveLSPDIntranetWanteds(ClassicPlayer player, int selectedCharId, string wanteds)
        {
            try
            {
                ...
                {
                    HUDHandler.SendNotification(policeMember, 1, 2500, $"{Characters.GetCharacterName(player.CharacterId)} hat die Akte von {Characters.GetCharacterName(selectedCharId)} bearbeitet.");
                }
            }
            catch (Exception e)
            {
        ...
        public void requestPoliceAppMostWanteds(ClassicPlayer player)
        {
            try
            {
                ...
                if (ServerFactions.GetCharacterFactionId(player.CharacterId) == 2 && ServerFactions.GetCharacterFactionRank(player.CharacterId) < 6) { HUDHandler.SendNotification(player, 3, 2500, "Keine Berechtigung"); return; }
        ...
        public void joinRadioFrequence(ClassicPlayer player, string radioFrequence)
        {
            try
            {
                ...
                if (radioFrequence == "911000" || radioFrequence == "911100" || radioFrequence == "911200" || radioFrequence == "911300")
                {
                    if (!ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                    {
                        HUDHandler.SendNotification(player, 3, 2500, "Dieser Funk ist Verschlüsselt!");
                        Characters.SetCharacterCurrentFunkFrequence(player.CharacterId, null);
                ...
                if (radioFrequence == "912000" || radioFrequence == "912100" || radioFrequence == "912200" || radioFrequence == "912300")
                {
                    if (!ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                    {
                        HUDHandler.SendNotification(player, 3, 2500, "Dieser Funk ist Verschlüsselt!");
                    ...
                    if (!ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != 3) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                    {
                        HUDHandler.SendNotification(player, 3, 2500, "Dieser Funk ist Verschlüsselt!");
                        Characters.SetCharacterCurrentFunkFrequence(player.CharacterId, null);
                        player.EmitLocked("Client:Smartphone:setCurrentFunkFrequence", "null");
                ...
                if (radioFrequence == "910000" || radioFrequence == "910100" || radioFrequence == "910200" || radioFrequence == "910300")
                {
                    if (!ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                    {
                        HUDHandler.SendNotification(player, 3, 2500, "Dieser Funk ist Verschlüsselt!");
                    ...
                    if (!ServerFactions.IsCharacterInAnyFaction(player.CharacterId) || (ServerFactions.GetCharacterFactionId(player.CharacterId) != 3 && ServerFactions.GetCharacterFactionId(player.CharacterId) != 2) || !ServerFactions.IsCharacterInFactionDuty(player.CharacterId))
                    {
                        HUDHandler.SendNotification(player, 3, 2500, "Dieser Funk ist Verschlüsselt!");
                        Characters.SetCharacterCurrentFunkFrequence(player.CharacterId, null);
                        player.EmitLocked("Client:Smartphone:setCurrentFunkFrequence", "null");
