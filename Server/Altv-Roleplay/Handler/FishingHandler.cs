...
namespace Altv_Roleplay.Handler
{
    ...
    {
        /*public static class fishPositions
        {
            //Sandy Shores
            public static readonly Position Sandy1 = new Position(713.5121f, 4092.6594f, 34.72229f);
        }*/
        ...
        internal static async Task startFishing(ClassicPlayer player)
        {
            try
            {
                if (player == null || !player.Exists || player.CharacterId <= 0) return;
                Alt.Log("Start Fishing");
                ...
                if (!CharactersInventory.ExistCharacterItem(charId, "Wurm", "inventory")) { HUDHandler.SendNotification(player, 3, 2500, "Du hast keine Köder dabei.."); return; }

                if (player.IsFishing() == true)
                {
                    HUDHandler.SendNotification(player, 3, 2500, "Du bist schon am Angeln!");
                    player.SetIsFishing(false);
                    return;
                }
                player.SetIsFishing(true);

                player.EmitLocked("Client:Inventory:AttachObject", "fishing-rod");
                CharactersInventory.RemoveCharacterItemAmount(charId, "Wurm", 1, "inventory");
                ...
                Random r = new Random();
                string[] words = { "Fisch", "Gras", "Wasser", "Feuerzeug" };
                string itemName = words[r.Next(0, words.Length)];
