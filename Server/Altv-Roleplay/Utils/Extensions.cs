...
namespace Altv_Roleplay.Utils
{
    public static partial class Extensions
    {
        ...
        {
            HUDHandler.SendNotification(player, 4, 2500, $"Du wurdest vom Server gekickt. Grund: {reason}");
            player.Kick(reason);
        }
        ...
        public static void SetVehicleHoodState(this IVehicle vehicle, bool isOpen) //True = offen, false = Zu
        {
            var myVehicle = (ClassicVehicle)vehicle;
            if (myVehicle == null || !myVehicle.Exists) return;
            myVehicle.Hoodstate = isOpen;
        }

        public static bool GetVehicleHoodState(this IVehicle vehicle)
        {
            var myVehicle = (ClassicVehicle)vehicle;
            if (myVehicle == null || !myVehicle.Exists) return false;
            return myVehicle.Hoodstate;
        }
        ...
        public static int AdminLevel(this IPlayer player)
        {
            ...
            var playerDb = User.Player.FirstOrDefault(p => p.hardwareId == player.HardwareIdHash && player.GetCharacterMetaId() == (ulong)p.Online);
        ...
        [AsyncClientEvent("Player:Meta:SetPlayerHasSeatbelt")]
        public static void SetPlayerHasSeatbelt(this IPlayer player, bool hasSeatbelt)
        {
            if (player == null || !player.Exists) return;
            AltAsync.Do(() => player.SetSyncedMetaData("HasSeatbelt", hasSeatbelt));
        }

        [AsyncClientEvent("Player:Meta:SetChat")]
        public static void SetChat(this IPlayer player, bool chatOpen)
        {
            if (player == null || !player.Exists) return;
            AltAsync.Do(() => player.SetSyncedMetaData("ChatOpen", chatOpen));
        }

        public static bool HasPlayerSeatbelt(this IPlayer player)
        {
            if (player == null || !player.Exists) return false;
            return player.GetSyncedMetaData("HasSeatbelt", out bool Seatbelt) ? Seatbelt : false;
        }
        [AsyncClientEvent("Player:Meta:SetIsProducing")]
        public static void SetIsProducing(this IPlayer player, bool Producing)
        {
            if (player == null || !player.Exists) return;
            AltAsync.Do(() => player.SetSyncedMetaData("IsProducing", Producing));
        }
        public static bool IsProducing(this IPlayer player)
        {
            if (player == null || !player.Exists) return false;
            return player.GetSyncedMetaData("IsProducing", out bool Producing) ? Producing : false;
        }

        [AsyncClientEvent("Player:Meta:SetIsFishing")]
        public static void SetIsFishing(this IPlayer player, bool Fishing)
        {
            if (player == null || !player.Exists) return;
            AltAsync.Do(() => player.SetSyncedMetaData("IsFishing", Fishing));
        }
        public static bool IsFishing(this IPlayer player)
        {
            if (player == null || !player.Exists) return false;
            return player.GetSyncedMetaData("IsFishing", out bool Fishing) ? Fishing : false;
        }

        [AsyncClientEvent("Player:Meta:SetCEFOpen")]
        public static void SetCEFOpen(this IPlayer player, bool cefOpen)
        {
            if (player == null || !player.Exists) return;
            AltAsync.Do(() => player.SetSyncedMetaData("IsCefOpen", cefOpen));
        }
        public static bool IsCEFOpen(this IPlayer player)
        {
            if (player == null || !player.Exists) return false;
            return player.GetSyncedMetaData("IsCefOpen", out bool cefOpen) ? cefOpen : false;
        }
